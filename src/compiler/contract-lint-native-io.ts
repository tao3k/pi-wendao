...
export type NativeInteractionType = Branded<string, "NativeInteractionType">;
...
export function readNativeInteraction(
...
export function readInputAssignment(
  task: Record<string, unknown>,
  inputName: string,
): string | undefined {
...
export function inputIdsByName(task: Record<string, unknown>, name: string): Set<string> {
  ...
  return new Set(
    readElements(io ?? {}, "dataInput")
      .filter((input) => readString(input.name) === name)
      .map((input) => readString(input.id)),
  );
}
...
export function outputIdsByName(task: Record<string, unknown>, name: string): Set<string> {
  const io = firstElement(task, "ioSpecification");
  return new Set(
    readElements(io ?? {}, "dataOutput")
      .filter((output) => readString(output.name) === name)
      .map((output) => readString(output.id)),
  );
}
