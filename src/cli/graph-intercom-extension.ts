...
Use send for fire-and-forget progress reports.`,
    ...
    async execute(toolCallId, params, signal) {
      ...
      if (action === "status") {
        return textResult("pi-wendao graph intercom is connected.", {
          action,
          delivered: true,
          mode: "pi-wendao-graph",
        });
      }
      if (action === "list") {
      ...
      if (action === "pending") {
        return textResult(
          "Pending graph-local asks are shown in the pi-wendao native chat stream.",
          { action, delivered: true },
        );
      }
      if (action === "reply") {
        bridge?.onEvent?.({ type: "intercom_reply", toolCallId, to: target, message, context });
        return textResult(`Reply recorded for ${target || "planner"}.`, {
          action,
          delivered: true,
          to: target || "planner",
        });
      ...
      if (action === "ask") {
        if (!bridge?.requestPlannerReply) {
          return textResult(
            "No pi-wendao graph planner inbox is available.",
        ...
        if (target && target !== "planner") {
          return textResult(
            `pi-wendao graph intercom can ask planner, not ${target}.`,
            { action, delivered: false, to: target },
            true,
          );
        }
      ...
      return textResult(
        "intercom action must be list, send, ask, reply, pending, or status",
        { action, delivered: false },
        true,
      );
    },
