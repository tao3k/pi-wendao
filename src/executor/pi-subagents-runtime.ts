...
export interface DiscoverPiSubagentsRuntimeHostOptions extends Omit<PiSubagentsRuntimeHostOptions, "loadResult"> {
	cwd: string;
	agentDir?: string;
...
export function createPiSubagentsClientFromRegisteredTools(
	...
	return {
		spawn: async (request, callbacks) => {
			const result = await executeRegisteredTool(tools.Agent!, "Agent", request, options, callbacks);
...
function buildRuntimeHostOptions(
	options: PiSubagentsRuntimeHostOptions,
): PiSubagentsHostOptions {
...
export function getCurrentPiSubagentsToolExecutionContext(): PiSubagentsToolExecutionContext | undefined {
...
function extractAgentIdResult(result: PiToolResultLike): PiSubagentsSpawnResult {
	...
	if (typeof details === "object" && details !== null) {
		const agentId = (details as { agentId?: unknown; agent_id?: unknown; id?: unknown }).agentId
