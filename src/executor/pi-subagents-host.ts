...
export interface PiSubagentsClient {
	spawn(request: PiSubagentsSpawnRequest, callbacks?: PiSubagentsClientCallbacks): Promise<PiSubagentsSpawnResult>;
	getResult(request: PiSubagentsGetResultRequest, callbacks?: PiSubagentsClientCallbacks): Promise<unknown>;
}
...
async function runPiSubagentTask(
	...
	if (stored?.status === "completed" && stored.output && hasRequiredOutputs(stored.output, config.outputs)) {
	...
	const agentId = reusableStored?.agentId ?? await spawnAndStoreSubagent(
		options,
		request,
		key,
		spawnRequest,
	...
	const result = await options.client.getResult({
		agent_id: agentId,
		wait: true,
		...(options.verboseResult === undefined && !options.onEvent ? {} : { verbose: options.verboseResult ?? true }),
	}, {
		activityId: request.activityId,
		description: spawnRequest.description,
		agentId,
	...
	if (agentError) {
		...
		throw new Error(`pi-subagents agent ${agentId} failed for ${request.activityId}: ${agentError}`);
	}
