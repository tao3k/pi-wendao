...
describe("pi-intercom runtime tool adapter", () => {
  ...
  it("throws agent-tool errors for pi-intercom failures", async () => {
    ...
    await expect(tool.execute("tool-1", { action: "status" })).rejects.toThrow(
  ...
  it("mirrors successful send and ask results into correlation state", async () => {
    ...
    expect(records.find((record) => record.status === "sent")?.context.message.id).toBe(
      "sent-message",
    );
    expect(records.find((record) => record.status === "replied")?.reply?.text).toContain("ready");
  });
