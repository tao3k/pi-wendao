...
describe("PiWendaoIntercomReplyTracker", () => {
  ...
  it("uses sender id or name to disambiguate multiple pending asks", () => {
    ...
    expect(tracker.resolveReplyTarget({ to: "reviewer", now: 1002 }).message.id).toBe(
      "ask-reviewer",
    );
    expect(tracker.resolveReplyTarget({ to: "session-planner", now: 1002 }).message.id).toBe(
      "ask-planner",
    );
  });
  ...
  it("expires pending asks after the configured timeout", () => {
    ...
    expect(tracker.pruneExpired(1051).map((context) => context.message.id)).toEqual([
      "ask-timeout",
    ]);
