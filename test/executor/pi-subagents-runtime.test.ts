...
describe("pi-subagents runtime tool adapter", () => {
	...
	it("creates a client from loaded extensions", async () => {
		...
		await expect(client.getResult({ agent_id: "loaded-agent", wait: true })).resolves.toBe("loaded result");
	});
	...
	it("creates a checkpoint-aware host from loaded extensions", async () => {
		...
		const cachedHost = createPiSubagentsHostFromLoadedExtensions({
			loadResult: {
				extensions: [{
					tools: new Map([
						["Agent", { definition: tool("Agent", async () => {
							called = true;
							throw new Error("cached runtime host should not spawn");
						}) }],
						["get_subagent_result", { definition: tool("get_subagent_result", async () => {
							called = true;
	...
	it("returns undefined when loaded extensions do not provide pi-subagents tools", () => {
		expect(tryCreatePiSubagentsHostFromLoadedExtensions({
...
`, "utf-8");
		...
		await expect(discovered.host!.run({
...
function tool(
	name: string,
	execute: (params: Record<string, unknown>, ctx: unknown) => Promise<{ content: Array<{ type: string; text?: string }>; details?: unknown }>,
