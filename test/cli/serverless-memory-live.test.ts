...
const itLive = process.env.RUN_PI_WENDAO_SERVERLESS_MEMORY_LIVE === "1" ? it : it.skip;

describe("serverless memory live recall", () => {
  itLive(
    "answers from injected recall context with a real model",
    async () => {
      const evidencePath =
      ...
      const result = await runServerlessMemoryLiveRecallSmoke({
        cwd: projectRoot(),
        packetPath:
          process.env.PI_WENDAO_SERVERLESS_MEMORY_LIVE_PACKET ?? taskListJsonFixturePath(),
        modelPattern:
          process.env.PI_WENDAO_SERVERLESS_MEMORY_LIVE_MODEL ?? "anthropic/deepseek-v4-pro",
