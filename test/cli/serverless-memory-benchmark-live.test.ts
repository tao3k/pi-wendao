...
const itLive = process.env.RUN_PI_WENDAO_SERVERLESS_MEMORY_BENCHMARK_LIVE === "1" ? it : it.skip;

describe("serverless memory live recall benchmark", () => {
  itLive(
    "compares section, property, and org-elements recall contexts",
    async () => {
      const evidencePath =
      ...
      const result = await runServerlessMemoryRecallBenchmark({
        cwd: projectRoot(),
        packetPath:
          process.env.PI_WENDAO_SERVERLESS_MEMORY_BENCHMARK_PACKET ?? taskListJsonFixturePath(),
        modelPattern:
          process.env.PI_WENDAO_SERVERLESS_MEMORY_BENCHMARK_MODEL ?? "anthropic/deepseek-v4-pro",
