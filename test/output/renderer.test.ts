...
describe("renderer event formatting", () => {
	...
	it("does not block when key input is not a TTY", async () => {
		...
		await expect(waitForTerminalKey(input as unknown as NodeJS.ReadStream)).resolves.toBeUndefined();
	...
	it("summarizes assistant JSON blocks instead of printing raw payloads", () => {
		const lines = formatAssistantMessageForLog('```json\n{"fileList":"a.ts\\nb.ts\\nc.ts"}\n```').map(stripAnsi);
	...
	it("summarizes multiline tool output", () => {
		const lines = formatToolResultForLog("bash", {
	...
	it("formats activity-scoped pi-subagents tool calls with arguments", () => {
		...
		expect(formatPiSubagentsHostToolEventForGraphDetail(call)).toBe("tool:bash \"find . -name \\\"*.ts\\\"\"");
		expect(formatPiSubagentsHostToolEventForLog({
			...call,
			type: "tool_result",
			content: [{ type: "text", text: "a.ts\nb.ts\n" }],
			isError: false,
		...
		expect(formatPiSubagentsHostToolEventForGraphDetail(intercomCall)).toBe("tool:intercom action=status");
	});
	...
	it("formats activity-scoped pi-subagents live updates for graph nodes", () => {
		...
		const lines = formatPiSubagentsToolUpdateForLog(update, { activityId: "Task_A" }).map(stripAnsi);
	...
	it("normalizes pi-subagents response text fragments in live updates", () => {
		...
		expect(formatPiSubagentsToolUpdateForLog(update, { activityId: "Task_A" }).map(stripAnsi))
